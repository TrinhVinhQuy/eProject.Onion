...
namespace WebASPMvcCore.DataAccess.Dapper
{
    public class SQLQueryHandler : ISQLQueryHandler
    {
        ...
        public async Task<IEnumerable<T>> ExecuteReturnListRowAsync<T>(string query, DynamicParameters parammeters = null,
                                                                            IDbTransaction dbTransaction = null)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                return await connection.QueryAsync<T>(query, param: parammeters, dbTransaction);
            }
        }
    }
}
